...
# ---------------------------------------------------------------------------
# AOSP repo-based resolution
# ---------------------------------------------------------------------------

def _resolve_one_aosp_repo(entry, android_version, bearer, bd_url,
                           trust_cert):
    info = entry["info"]
...
def resolve_aosp_repo_packages(packages_by_tier, android_version, bearer,
    ...
    for f in futures:
        ...
        if result:
            ...
            aosp_name = result["aosp_name"]
            purl = f"pkg:android/{aosp_name}@{android_version}"

            for ref in pkg["externalRefs"]:
                if ref["referenceType"] == "purl":
                    ref["referenceLocator"] = purl
                    break
            pkg["versionInfo"] = android_version
            ...
            entry["bd_ref"] = {"comp_id": result["comp_id"],
                               "ver_id": result["ver_id"],
                               "origin_id": result["origin_id"]}
            info["version_source"] = "aosp_repo_purl"
            resolved += 1
            print(f"  AOSP matched: {pkg_name} -> {purl}",
                  file=sys.stderr)
